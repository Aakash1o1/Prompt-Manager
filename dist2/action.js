(() => {
  ...
  async function setHosts(hs) {
    return new Promise((res) => chrome.storage.local.set({ [STORAGE_KEY]: hs }, () => res()));
  }
  ...
  function requestPermissionForPattern(pattern) {
  ...
  async function renderPopularSites() {
    ...
    const checks = await Promise.all(POPULAR_SITES.map((s) => new Promise((res) => {
  ...
  function confirmPermissionUi(pattern) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    btn.addEventListener("click", async () => {
      ...
      for (const pattern of originsToRequest) {
        ...
        await setHosts(Array.from(/* @__PURE__ */ new Set([...cur, pattern])));
