    ...
    <style>
      ...
      */
      ...
      .panel.mode-add .controls {
        /* Make the controls container take up all available space in the header */
        flex-grow: 1;
      }

      .panel.mode-add .controls #add-btn,
      .panel.mode-add .controls #settings-btn {
        /* Hide the Add and Settings buttons */
        display: none;
      }

      .panel.mode-add .controls #tags-btn {
        /* Make the Tags button expand to fill the empty space */
        flex-grow: 1;
      }
  ...
  async function renderUI(opts) {
    ...
    tagsNewBtn.addEventListener("mousedown", (ev) => {
      ...
      save.addEventListener("click", async (e) => {
        ...
        const newTag = { id: uid(), name: nm, color, order: tags.length };
        tags.push(newTag);
        await saveTags();
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      if (editingId != null) {
        ...
        const newPrompt = { id: uid(), title, quick, text, tags: Array.from(new Set(draftPromptTagIds || [])) };
